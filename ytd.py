            ...
            try:
                for dep in missing:
                    if os.name == 'nt':
                        ...
                        subprocess.check_call(["sudo", "apt-get", "install", dep, "-y"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def get_youtube_url():
    """Get and validate the YouTube URL from command-line arguments."""
    if len(sys.argv) > 1:
        url = sys.argv[1]
